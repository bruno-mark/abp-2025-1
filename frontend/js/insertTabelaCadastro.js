// Script para exibição dos dados na tabela
...
document.getElementById("filtrar_btn").addEventListener("click", async () => {
  ...
  try {
    ...
    dados.forEach((item) => {
      ...
      const materias = [item.nome_disciplina];

      const seletor = document.querySelector(".seletorTabela")
      ...
      if (celula.innerHTML.trim() !== "") {
        ...
        celula.innerHTML = `<select>${conteudo}</select>`;
      }
    });
    ...
    // Código para preencher a tabela e docentes
  } catch (erro) {
